...
static int mpeg_ts_write_section_header(const mpeg_ts_enc_context_t *ts, int pid, unsigned int *cc,
...
static int ts_write_pes(mpeg_ts_enc_context_t *tsctx, const struct pmt_t *pmt, struct pes_t *stream,
    ...
    while (bytes > 0) {
        ...
        if (start) {
            ...
            if (PSI_STREAM_H264 == stream->codecid &&
                -1 == find_h264_access_unit_delimiter(payload, bytes)) {
                ...
                p += 6;
            } else if (PSI_STREAM_H265 == stream->codecid &&
                       -1 == find_h265_access_unit_delimiter(payload, bytes)) {
...
int mpeg_ts_write(void *ts, int pid, int flags, int64_t pts, int64_t dts, const void *data,
                  size_t bytes) {
    ...
    if (0x1FFF == pmt->PCR_PID ||
        (PES_SID_VIDEO == (stream->sid & PES_SID_VIDEO) && pmt->PCR_PID != stream->pid)) {
    ...
    if (0 == tsctx->pat_period ||
        tsctx->pat_period + PAT_PERIOD <= dts ||
        (++tsctx->pat_packet_count == tsctx->pat_packet_period)) {
        ...
        mpeg_ts_write_section_header(ts, PAT_TID_PAS, &tsctx->pat.cc, tsctx->payload,
                                     r); // PID = 0x00 program association table

        // PMT(Transport stream program map section)
        for (i = 0; i < tsctx->pat.pmt_count; i++) {
            r = pmt_write(&tsctx->pat.pmts[i], tsctx->payload);
            mpeg_ts_write_section_header(ts, tsctx->pat.pmts[i].pid, &tsctx->pat.pmts[i].cc,
                                         tsctx->payload, r);
        }
    }
